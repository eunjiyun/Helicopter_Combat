...
MATERIALSLOADINFO* CGameModelObj::LoadMaterialsInfoFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, FILE* pInFile)
{
	char pstrToken[64] = { '\0' };
	UINT nReads = 0;
...
CGameModelObj* CGameModelObj::LoadFrameHierarchyFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, FILE* pInFile)
{
	...
	CGameModelObj* pGameObject = NULL;
	...
	for (; ; )
	{
		...
		else if (!strcmp(pstrToken, "<Children>:"))
		{
			...
			if (nChilds > 0)
			{
				...
				{
					CGameModelObj* pChild = CGameModelObj::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pInFile);
...
void CGameModelObj::PrintFrameInfo(CGameModelObj* pGameObject, CGameModelObj* pParent)
{
	TCHAR pstrDebug[256] = { 0 };
...
CGameModelObj* CGameModelObj::LoadGeometryFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, char* pstrFileName)
{
	...
	for (; ; )
	{
		...
		{
			pGameObject = CGameModelObj::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pInFile);
		}
	...
	CGameModelObj::PrintFrameInfo(pGameObject, NULL);
#endif
...
//22.10.12
//모델로드를 위한 게임오브젝트
void CGameModelObj::SetMesh(CMesh* pMesh)
{
...
void CGameModelObj::SetChild(CGameModelObj* pChild, bool bReferenceUpdate)
{
	if (pChild)
	{
...
void CHellicopterObject::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	...
	CGameModelObj::Animate(fTimeElapsed, pxmf4x4Parent);
}
...
void CGameModelObj::SetShader(CShader* pShader)
{
	m_nMaterials = 1;
...
void CGameModelObj::SetShader(int nMaterial, CShader* pShader)
{
	if (m_ppMaterials[nMaterial]) m_ppMaterials[nMaterial]->SetShader(pShader);
}

void CGameModelObj::SetMaterial(int nMaterial, CMaterial* pMaterial)
{
...
void CGameModelObj::Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent)
{
	if (m_pSibling) m_pSibling->Animate(fTimeElapsed, pxmf4x4Parent);
	if (m_pChild) m_pChild->Animate(fTimeElapsed, &m_xmf4x4World);
}

CGameModelObj* CGameModelObj::FindFrame(char* pstrFrameName)
{
	CGameModelObj* pFrameObject = NULL;
...
void CGameModelObj::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	OnPrepareRender();
...
void CGameModelObj::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void CGameModelObj::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
}

void CGameModelObj::UpdateShaderVariable(ID3D12GraphicsCommandList* pd3dCommandList, XMFLOAT4X4* pxmf4x4World)
{
...
void CGameModelObj::UpdateShaderVariable(ID3D12GraphicsCommandList* pd3dCommandList, CMaterial* pMaterial)
{
}

void CGameModelObj::ReleaseShaderVariables()
{
}

void CGameModelObj::ReleaseUploadBuffers()
{
...
void CGameModelObj::UpdateTransform(XMFLOAT4X4* pxmf4x4Parent)
{
...
void CGameModelObj::SetPosition(float x, float y, float z)
{
...
void CGameModelObj::SetPosition(XMFLOAT3 xmf3Position)
{
	SetPosition(xmf3Position.x, xmf3Position.y, xmf3Position.z);
}

void CGameModelObj::SetScale(float x, float y, float z)
{
...
XMFLOAT3 CGameModelObj::GetPosition()
{
	return(XMFLOAT3(m_xmf4x4World._41, m_xmf4x4World._42, m_xmf4x4World._43));
}

XMFLOAT3 CGameModelObj::GetLook()
{
	return(Vector3::Normalize(XMFLOAT3(m_xmf4x4World._31, m_xmf4x4World._32, m_xmf4x4World._33)));
}

XMFLOAT3 CGameModelObj::GetUp()
{
	return(Vector3::Normalize(XMFLOAT3(m_xmf4x4World._21, m_xmf4x4World._22, m_xmf4x4World._23)));
}

XMFLOAT3 CGameModelObj::GetRight()
{
	return(Vector3::Normalize(XMFLOAT3(m_xmf4x4World._11, m_xmf4x4World._12, m_xmf4x4World._13)));
}

void CGameModelObj::MoveStrafe(float fDistance)
{
	XMFLOAT3 xmf3Position = GetPosition();
	XMFLOAT3 xmf3Right = GetRight();
	xmf3Position = Vector3::Add(xmf3Position, xmf3Right, fDistance);
	CGameModelObj::SetPosition(xmf3Position);
}

void CGameModelObj::MoveUp(float fDistance)
{
	XMFLOAT3 xmf3Position = GetPosition();
	XMFLOAT3 xmf3Up = GetUp();
	xmf3Position = Vector3::Add(xmf3Position, xmf3Up, fDistance);
...
void CGameModelObj::MoveForward(float fDistance)
{
	XMFLOAT3 xmf3Position = GetPosition();
	XMFLOAT3 xmf3Look = GetLook();
	xmf3Position = Vector3::Add(xmf3Position, xmf3Look, fDistance);
...
void CGameModelObj::Rotate(float fPitch, float fYaw, float fRoll)
{
	XMMATRIX mtxRotate = XMMatrixRotationRollPitchYaw(XMConvertToRadians(fPitch), XMConvertToRadians(fYaw), XMConvertToRadians(fRoll));
...
void CGameModelObj::Rotate(XMFLOAT3* pxmf3Axis, float fAngle)
{
	XMMATRIX mtxRotate = XMMatrixRotationAxis(XMLoadFloat3(pxmf3Axis), XMConvertToRadians(fAngle));
	m_xmf4x4Transform = Matrix4x4::Multiply(mtxRotate, m_xmf4x4Transform);
...
void CGameModelObj::Rotate(XMFLOAT4* pxmf4Quaternion)
{
