...
//	void LoadBufferFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, wchar_t *pszFileName, UINT nIndex);
	void LoadBuffer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pData, UINT nElements, UINT nStride, 
		DXGI_FORMAT ndxgiFormat, UINT nIndex);
...
public:
	...
	//22.10.13
	//static ID3D12Resource* m_pd3dcbGameObject;// = NULL;
	//ID3D12Resource* m_pd3dcbGameObject= NULL;
	//
};
...
public:
	CGameModelObj();
	virtual ~CGameModelObj();

public:
	//22.05.28
	CGameModelObj* m_pObjectCollided = NULL;
	...
	CGameModelObj* m_pParent = NULL;
	CGameModelObj* m_pChild = NULL;
	CGameModelObj* m_pSibling = NULL;
	//22.10.12
	bool							canDo = false;
	//
	...
	void SetChild(CGameModelObj* pChild, bool bReferenceUpdate = false);
	...
	CGameModelObj* GetParent() { return(m_pParent); }
	void UpdateTransform(XMFLOAT4X4* pxmf4x4Parent = NULL);
	CGameModelObj* FindFrame(char* pstrFrameName);
...
public:
	...
	//static CMeshLoadInfo* LoadMeshInfoFromFile(char* pstrFileName);
	...
	static void PrintFrameInfo(CGameModelObj* pGameObject, CGameModelObj* pParent);

	//22.10.12
	//loadgeo함수 추가
	static CGameModelObj* LoadGeometryFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, char* pstrFileName);
	static CGameModelObj* LoadFrameHierarchyFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, FILE* pInFile);
	//static CMeshLoadInfo* LoadMeshInfoFromFile(FILE* pInFile);
	//
...
class CHellicopterObject : public CGameModelObj
{
public:
	CHellicopterObject();
...
protected:
	CGameModelObj* m_pMainRotorFrame = NULL;
	CGameModelObj* m_pTailRotorFrame = NULL;
...
public:
	...
	virtual void CommandF4(CGameModelObj* m_pPlayer);
